...
foreach ($projectFile in $projectFiles) {
    ...
    foreach ($reference in $project.SelectNodes('//ProjectReference')) {
        ...
        if ($include -match '\$\(GmaModule(?!TaskRuntimeRoot\))') {
...
foreach ($sourceFile in $sourceFiles) {
    ...
    if ($source -match 'Gma\.Modules\.(?!TaskRuntime(?:\.|;))') {
...
if ($errors.Count -gt 0) {
    throw "TaskRuntime boundary checks failed:`n - $($errors -join "`n - ")"
}

Write-Host 'TaskRuntime boundary checks passed.'
